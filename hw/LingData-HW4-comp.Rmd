...
```{r}
howmanyparticipants <- length(table(mono_socio$ParticipantID))
howmanyparticipants

responces <- table(mono_socio$Responce)
responces[names(responces)=='male']
responces[names(responces)=='female']
...
edu <- mono_socio %>%
  filter(QuestionCategory=='education')
diff_edu <- table(edu$Responce)
diff_edu

responces[names(responces)=='right']
responces[names(responces)=='left']
```
...
No, we can't state this because it is too much moved to the left and the right tail is way too long.
...
Yes, we can say so. The overall skewness is more similar to that of a normal distribution.
...
mono1 <- 
mono %>% 
  filter(RT > 600 & abs(RTlog - mean(mono$RTlog) < 3 * sd(mono$RTlog) & TypeOfStimuli != 'FillerAcceptable' & TypeOfStimuli != 'FillerUnacceptable')) %>%
...
ggplot(fig, aes(x = TypeOfStimuli, y = Freq, fill = AcceptabilityJ)) + 
geom_col(colour = "black", position = "dodge") + facet_grid(vars(WordOrder))
```
...
ggplot(fig2, aes(x = WordOrder, y = Freq, fill = AcceptabilityJ)) + 
...
ggplot(mono1, aes(x = AcceptabilityJ, y = RTlog, fill = WordOrder)) +   geom_violin(position = position_dodge(1)) + geom_boxplot(width=0.3, position = position_dodge(1))
...
library(ggpubr)
ggline(mono1, x = "AcceptabilityJ", y = "RTlog", add = "mean_se", color = "WordOrder")
```
...
mean_rtlog_long <- pivot_wider(mean_rtlog_long, names_from = 'WordOrder', values_from = RTlog)
mean_rtlog_long
```
...
The difference between the results might be caused by the fact that in 2.3 we compared different types of stimuli over all the dataframe, and in this case we compared difference between stimuli types for every certain person. This might be explained simply by the fact that some people react faster or slower than the rest.
...
```{r}
chi <- chisq.test(incong_rtlog$TypeOfStimuli, incong_rtlog$RTlog)
chi

```

I won't reject the H0 because p-value is too big.
...
TukeyHSD(chi)
```
There was an error so I couldn't apply this method.
